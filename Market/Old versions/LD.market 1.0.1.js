		...
		.reduce((countNotProfitOrders, currentOrder) =>
		{
			...
			if (profit === undefined)
			{
				showMessage({
					...
					color: warningColor, timeout: 60 * 1000,
				});
...
class Item
{
	...
	async calcBuyPriceOrder(buyBook)
	{
		...
		else if (calcProfit(itemOrders.orders.buying[0].price.amount + 0.01, averagePrice) >= settings.minProfitOrder(averagePrice))
		{
			priceToOrder = itemOrders.orders.buying[0].price.amount + 0.01;
		} else
		{
			// Проверка остальных запросов на покупку, если их меньше трех
			for(const buying of itemOrders.orders.buying.slice(0, 3))
			{
				if (buying.number >= 2)
				{
					break;
		...
		for(const order of buyBooks.results)
		{
			...
			if (!DEBUG_ORDER)
			{
				if (orderPrice === -1 || // Если предмет не выгодный, то убрать ордер
					orderPrice !== order.price.amount + 0.01) // Если предмет выгодный и цена ордера изменилась
				{
